...
# This loads variables from a .env file located in the current working directory
load_dotenv()
...
async def test_bearer_token():
    token = await get_bearer_token()  # ✅ no arguments
    assert token.startswith("ey"), "Bearer token does not look like JWT"

@pytest.mark.asyncio
async def test_get_tenant_details_valid():
    """
    Test retrieving tenant details using the bearer token.
    """
    # Use the already awaited bearer_token fixture directly.
    details_str = await get_tenant_details()
...
@pytest.mark.asyncio
async def test_list_connections_valid():
    """
    ...
    response_str = await list_connections(limit=10)
...
@pytest.mark.asyncio
async def test_create_schema_valid():
    """
...
@pytest.mark.asyncio
async def test_get_schema_by_id_valid():
    """
    ...
    response_str = await get_schema_by_id(schema_id=schema_id)
...
@pytest.mark.asyncio
async def test_create_credential_definition_valid():
    """
...
@pytest.mark.asyncio
async def test_create_out_of_band_invitation():
    """
